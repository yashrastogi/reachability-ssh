...
def ping(client, IPp):
    ...
    # chan.exec_command(f"ping -c1 -w1 {IPp}")
    chan.exec_command(f"timeout 0.2 ping -c1 {IPp}")
...
def traceroute(client, TTL, IPp):
    ...
    for lin in stdout:
        traceOut += lin
    if len(re.findall(IPp, traceOut)) < 3:
...
with open(targetPath, newline="") as targets:
    reachCSV = open(
        os.path.join(".", "reports", f"{os.path.splitext(targets.name)[0]}_{IP}.csv"),
        "w",
        newline="",
    )
    ...
    timerThread = threading.Thread(target=timeCounter)
    timerThread.start()
